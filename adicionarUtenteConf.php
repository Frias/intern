...
function formRegUt($nome_tmp, $nomeErro, $tipoID_tmp, $numID_tmp, $idErro,$tipoCartUt_tmp, $numCartUt_tmp, $cartErro,$tipoSangue_tmp, $morada_tmp, $moradaErro, $contacto_tmp, $obs_tmp){	
		echo '
        ...
        <?php
			...
			if(empty($numID_tmp)) {
				$idErro=" * Preencha este campo!";
						...
						//Verifica se o n.º de cartão de utente já existe na BD
						...
						if ($verRes) { //verifica se a variável tem conteúdo
							while ($reg=mysqli_fetch_assoc($verRes)) { //Imprime o conteúdo do array
								if ($reg["numID"]==$numID_tmp){ //verifica se o ID de utente que está a ser registado já existe na BD
				...
				$cartErro=" * Preencha este campo!";
						...
						//Verifica se o n.ç de cartão de utente já existe na BD
						$sql="SELECT * FROM t_utentes"; //variável com comando SQL que selecciona todos os campos
						$verRes = mysqli_query ($cn,$sql); //guarda os dados vindos da tabela
						if ($verRes) { //verifica se a variável tem conteúdo
							while ($reg=mysqli_fetch_assoc($verRes)) { //Imprime o conteúdo do array
								if ($reg["numCartUtente"]==$numCartUt_tmp){ //verifica se o ID de utente que está a ser registado já existe na BD
			...
			if((!empty($nomeErro)) || (!empty($idErro))|| (!empty($cartErro))|| (!empty($moradaErro))) { //caso existam mensagens de erros, executa a linha abaixo imprimindo o formulário dentro da função formRegUtil
					formRegUt($nome_tmp, $nomeErro, $tipoID_tmp, $numID_tmp, $idErro,$tipoCartUt_tmp, $numCartUt_tmp, $cartErro,$tipoSangue_tmp, $morada_tmp, $moradaErro, $contacto_tmp, $obs_tmp);	//Em caso de erro, imprime o conteúdo da função dando informação do tipo de erro no próprio formulário
				}
					...
					$sql="INSERT INTO t_utentes (nome,tipoID,numID,tipoCartUtente,numCartUtente,tipoSangue, morada,contacto,obs) VALUES ('$nome_tmp','$tipoID_tmp','$numID_tmp','$tipoCartUt_tmp','$numCartUt_tmp','$tipoSangue_tmp','$morada_tmp','$contacto_tmp','$obs_tmp')"; //Com password encryptada SHA
