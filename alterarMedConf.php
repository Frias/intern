...
<?php
function formRegMed($cod_tmp, $nome_tmp, $nomeErro, $tipoID_tmp, $numID_tmp, $idErro, $morada_tmp, $moradaErro, $contacto_tmp, $obs_tmp, $esp_tmp, $tesp){	
		echo '
			 <form id="formRegMed" name="formRegMed" method="post" action="alterarMedConf.php">   
				<tr>
    ...
    <td colspan="3" id="bodyCab">
        ...
        <?php
			...
			if(empty($numID_tmp)) {
				$idErro=" * Preencha este campo!";
						...
						if ($verRes) { //verifica se a variável tem conteúdo
							while ($reg=mysqli_fetch_assoc($verRes)) { //Imprime o conteúdo do array
								if (($reg["numID"]==$numID_tmp)&&($reg["codMed"]!=$cod_tmp)){ //verifica se o ID de médico que está a ser registado já existe na BD
				...
				//verifica preencheu o campo referente à morada
				if(empty($morada_tmp)) {
					$moradaErro=" * Inserir uma morada de médico!";
					}
					elseif((strlen($morada_tmp)<9)){// Senão se a morada tiver menos de 9 caracteres, executa a linha abaixo
			...
			if((!empty($nomeErro)) || (!empty($idErro))|| (!empty($moradaErro))) { //caso existam mensagens de erros, executa a linha abaixo imprimindo o formulário dentro da função formRegUtil
					...
					formRegMed($cod_tmp,$nome_tmp, $nomeErro, $tipoID_tmp, $numID_tmp, $idErro, $morada_tmp, $moradaErro, $contacto_tmp, $obs_tmp, $esp_tmp, $tesp);	//Em caso de erro, imprime o conteúdo da função dando informação do tipo de erro no próprio formulário
				}
				else { //caso não existam mensagens de erros, executa as linhas abaixo inserindo os dados na BD na tabela t_medicos
					//Insere na BD
					ob_start(); //Ativa o buffer de saída para evitar alguns erros
					$sql="UPDATE t_medicos SET nome='".$nome_tmp."',tipoID='".$tipoID_tmp."',numID='".$numID_tmp."',morada='".$morada_tmp."',contacto='".$contacto_tmp."',obs='".$obs_tmp."', esp='".$esp_tmp."' WHERE codMed = '".$cod_tmp."'"; 
