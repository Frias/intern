...
<?php
function formRegUtil($cod, $nome, $nomeErro, $user, $userErro, $pass, $passErro, $passConfErro, $email, $emailErro, $tipoUtil,$dataReg){	
	echo '
			<form id="formEdit" name="formEdit" method="post" action="alterarUtilConf.php">
            <tr>
              <td class="tabFormEsq" width="150">Código:</td>
              <td width="650" class="msgErro">
              <input type="text" name="cod" id="cod" value="'.$cod.'" class="tabFormDir" readonly="readonly"/></td>
            </tr>
            <tr>
            ...
            <tr>
              ...
              <td>
              <input type="password" name="pass" id="pass" value="'.$pass.'"  class="tabFormDir"/> ** <div class="msgErro">'.$passErro.'</div></td>
            </tr>
            <tr>
            ...
            <tr>
              ...
              <td>
                 <select name="tipoUtil" id="tipoUtil" class="tabFormDir">';
					if ($tipoUtil=="Administrador"){ //Verifica qual o tipo de utilizador para saber qual deverá ficar selecionado na caixa de seleção
						print "<option value=\"Administrador\" selected=\"selected\">Administrador</option>
            ...
            <tr>
              <td align="right" class="tabFormEsq">Data de registo:</td>
              <td>
              <input type="text" name="dataReg" id="dataReg" value="'.$dataReg.'" readonly="readonly" class="tabFormDir" /></td>
            </tr>			
            <tr>
              <td class="tabFormEsq">&nbsp;</td>
            ...
            <tr bgcolor="#CCCCCC">
              <td class="tabFormEsq"><b>** NOTA:</b></td>			
              <td colspan="2" class="msgErro">&nbsp;&nbsp; Para alterar a palavra passe, deve inserir uma nova e depois confirmar!!!</td>
            </tr>			
        </form>';
}
  ...
  <tr>
    <td colspan="3" id="bodyCab">
                            ...
                            <td width="5" bgcolor="#FFFFFF"></td>                              
                                <td bgcolor="#FFFFFF" align="center">
									<table width="600" border="0" cellspacing="1" cellpadding="1" align="center">
                                        <tr><td id="textoCabTabCorpo" colspan="9">Alterar Dados de  Utilizador</td></tr>
        ...
        <?php
					...
					$userErro=" * O nome de utilizador deve ter entre 6 e 15 caracteres!";
						...
						if ($verRes) { //verifica se a variável tem conteúdo
							while ($reg=mysqli_fetch_assoc($verRes)) { //Imprime o conteúdo do array
								if (($reg["user"]==$user_tmp)&&($reg["codUtil"]!=$cod_tmp)){ //verifica se o novo nome de utilizador que está a ser alterado já existe na BD
			...
			// verifica o campo da palavra passe também é para ser alterado
			if(!empty($pass_tmp)&&(strlen($pass_tmp)<6) || (strlen($pass_tmp)>15)){ //Se a pass não estiver vazia e tiver menos de 6 ou mais de 15 caracteres, executa a linha abaixo
			...
			if((!empty($nomeErro)) || (!empty($emailErro))|| (!empty($passErro))|| (!empty($userErro))) { //caso existam mensagens de erros, executa a linha abaixo imprimindo o formulário dentro da função formRegUtil
					formRegUtil($cod_tmp, $nome_tmp, $nomeErro, $user_tmp, $userErro, $pass_tmp, $passErro, $passConfErro, $email_tmp, $emailErro, $tipoUtil_tmp, $dataReg_tmp);	//Em caso de erro, imprime o conteúdo da função dando informação do tipo de erro no próprio formulário
				}
				else { //caso não existam mensagens de erros, executa as linhas abaixo inserindo os dados na BD na tabela t_utilizadores
					//Altera dados na BD
					if ($pass_tmp==NULL){//Se o campo pass não estiver preenchido, não altera a pass na BD
						$sql="UPDATE t_utilizador SET nomeCompUtil='".$nome_tmp."',user='".$user_tmp."',email='".$email_tmp."',tipoUtil='".$tipoUtil_tmp."',dataRegUtil='".$dataReg_tmp."' WHERE codUtil = '".$cod_tmp."'";
					}else{ //senão se o campo pass estiver preenchido, guarda a pass na BD
						$sql="UPDATE t_utilizador SET nomeCompUtil='".$nome_tmp."',user='".$user_tmp."',pass=SHA('".$pass_tmp."'),email='".$email_tmp."',tipoUtil='".$tipoUtil_tmp."',dataRegUtil='".$dataReg_tmp."' WHERE codUtil = '".$cod_tmp."'";
					}
